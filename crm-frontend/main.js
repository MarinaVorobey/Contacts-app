(() => {
  ...
  const CLIENTS_LIST = [
    ...
    },
    {
      ...
      contacts: [
        ...
        {
          type: 'Twitter',
          value: 'https://twitter.com/cuprianov'
        },
        {
          type: 'Email',
          value: 'ytujik@xyz.com'
  ...
  function getContact(formattingContact) {
    ...
    contactWrapper.classList.add('table__contact');
    contactWrapper.setAttribute('target', '_blank');
    ...
    switch (formattingContact.type) {
      case 'Телефон': contactWrapper.innerHTML =
      ...
      contactWrapper.href = `tel:${formattingContact.value}`;
      tooltipText.textContent = formattingContact.value;
      break;
      case 'Email': contactWrapper.innerHTML =
        `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g id="mail">
            <path id="Subtract" opacity="0.7" fill-rule="evenodd" clip-rule="evenodd" d="M8 16C12.4183 16 16 12.4183 16 8C16 3.58172 12.4183 0 8 0C3.58172 0 0 3.58172 0 8C0 12.4183 3.58172 16 8 16ZM4 5.75C4 5.3375 4.36 5 4.8 5H11.2C11.64 5 12 5.3375 12 5.75V10.25C12 10.6625 11.64 11 11.2 11H4.8C4.36 11 4 10.6625 4 10.25V5.75ZM8.424 8.1275L11.04 6.59375C11.14 6.53375 11.2 6.4325 11.2 6.32375C11.2 6.0725 10.908 5.9225 10.68 6.05375L8 7.625L5.32 6.05375C5.092 5.9225 4.8 6.0725 4.8 6.32375C4.8 6.4325 4.86 6.53375 4.96 6.59375L7.576 8.1275C7.836 8.28125 8.164 8.28125 8.424 8.1275Z" fill="#9873FF"/>
          </g>
        </svg>`;
      contactWrapper.href = `mailto:${formattingContact.value}`;
      tooltipText.textContent = formattingContact.value;
      break;
      case 'Facebook': contactWrapper.innerHTML =
        `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g id="fb" opacity="0.7">
            <path id="fb_2" d="M7.99999 0C3.6 0 0 3.60643 0 8.04819C0 12.0643 2.928 15.3976 6.75199 16V10.3775H4.71999V8.04819H6.75199V6.27309C6.75199 4.25703 7.94399 3.14859 9.77599 3.14859C10.648 3.14859 11.56 3.30121 11.56 3.30121V5.28514H10.552C9.55999 5.28514 9.24799 5.90362 9.24799 6.53815V8.04819H11.472L11.112 10.3775H9.24799V16C11.1331 15.7011 12.8497 14.7354 14.0879 13.2772C15.3261 11.819 16.0043 9.96437 16 8.04819C16 3.60643 12.4 0 7.99999 0Z" fill="#9873FF"/>
          </g>
        </svg>`;
      contactWrapper.href = `${formattingContact.value}`;
      tooltipText.textContent = `Facebook: ${formattingContact.value}`;
      break;
      case 'VK': contactWrapper.innerHTML =
        `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g id="vk" opacity="0.7">
            <g id="vk_2">
      ...
      contactWrapper.href = `${formattingContact.value}`;
      tooltipText.textContent = `VK: ${formattingContact.value}`;
      break;
      default: contactWrapper.innerHTML =
        `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g id="mail">
      ...
      contactWrapper.href = `${formattingContact.value}`;
      tooltipText.textContent = `${formattingContact.type}: ${formattingContact.value}`;
      break;
    }
    tooltipBlock.append(tooltipText);
    contactWrapper.append(tooltipBlock);
  ...
  function renderClientsTable(clientsArray) {
    ...
    for (let i = 0; i < clientsArray.length; i++) {
      ...
      for (let k = 0; k < contactsData.length; k++) {
        ...
        if (k > 3) {
          contactFormatted.classList.add('visually-hidden');
        }
        contactsBlock.append(contactFormatted);
      }
      td5.append(contactsBlock);

      if (td5.querySelectorAll('.table__contact').length > 4) {
        const contactPlus = document.createElement('button');
        contactPlus.classList.add('table__contacts-plus', 'btn-reset');
        contactPlus.textContent = `+${td5.getElementsByClassName('table__contact').length - 4}`;
